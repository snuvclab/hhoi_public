...
def annotate_camera_train(data_dir):
    ...
    for img_path in tqdm(camera_train):
        ...
        for i in tqdm(range(len(class_ids))):
            if class_ids[i] == 6:
...
def annotate_real_train(data_dir):
    ...
    for inst_path in tqdm(sorted(path_to_size)):
...
def annotate_test_data(data_dir):
    ...
    for source, img_list, intrinsics, subset in tqdm(subset_meta):
        valid_img_list = []
