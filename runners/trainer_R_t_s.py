...
from dataset.datasets_R_t_s import get_data_loaders_from_cfg, process_batch
...
def train_score(cfg, train_loader, score_agent):
    ...
    for epoch in range(score_agent.clock.epoch, cfg.n_epochs):
        ...
        for i, batch_sample in enumerate(pbar):
            ...
            if cfg.model_type == 'hoi':
                with torch.no_grad():
                    human_pose_env_vec = model.encoder(batch_sample['gt_pose_scale'][:, 10:])
                batch_sample['gt_pose_scale'] = torch.cat((batch_sample['gt_pose_scale'][:, :10], human_pose_env_vec), dim=-1) # 6 + 3 + 1 + human_pose_dim
            elif cfg.model_type == 'hhi':
                with torch.no_grad():
                    p1_vec = model.encoder(batch_sample['gt_pose_scale'][:, :126])
                    p2_vec = model.encoder(batch_sample['gt_pose_scale'][:, -126:])
                batch_sample['gt_pose_scale'] = torch.cat(( p1_vec, batch_sample['gt_pose_scale'][:, 126:-126], p2_vec), dim=-1) # human_pose_dim + 6 + 3 + human_pose_dim
            else:
                raise NotImplementedError
            ...
            for i in range(len(data_modes)):
                ...
                for i, sampler_mode in enumerate(sampler_mode_list):
                    ...
                    if cfg.normalize_data:
                        gt_mean = test_batch["gt_mean"][i]
                        gt_std = test_batch['gt_std'][i]
                        res_normal = (res_list[i] * gt_std) + gt_mean
                        ...
                        # target_rot_mat = get_rot_matrix(res_list[i][:, 9:15], cfg.pose_mode).cpu().numpy()
                        # target_trans = res_list[i][:, 15:].cpu().numpy()
...
def main():
    # load config
    cfg = get_config()
    wandb.init(project="HHOI",
               name=f"{cfg.log_dir}",
               config={
                   "learning_rate": cfg.lr,
                   "weight_decay": cfg.lr_decay
			   })
