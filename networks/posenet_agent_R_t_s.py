...
from networks.gf_algorithms.losses_R_t_s import loss_fn, loss_fn_edm
from networks.gf_algorithms.sde import init_sde
from networks.posenet_R_t_s import GFObjectPose
...
class PoseNet(nn.Module):
    ...
    def load_ckpt_for_hhoi(self, hoi_model_path, hhi_model_path):
        """load checkpoint from saved checkpoint"""

        hoi_ckpt = torch.load(hoi_model_path)
        hhi_ckpt = torch.load(hhi_model_path)
        
        hoi_sd = hoi_ckpt['model_state_dict']
        hhi_sd = hhi_ckpt['model_state_dict']
        
        print(f"Loading checkpoint from {hoi_model_path} and {hhi_model_path} ...")
        
        merged_sd = {}
        for k, v in hoi_sd.items():
            if k.startswith('pose_score_net.'):
                new_k = k.replace('pose_score_net.', 'pose_score_net.hoi_')
            else:
                new_k = k
            merged_sd[new_k] = v
        for k, v in hhi_sd.items():
            if k.startswith('pose_score_net.'):
                new_k = k.replace('pose_score_net.', 'pose_score_net.hhi_')
            else:
                new_k = k
            merged_sd[new_k] = v
            
        own_sd = (self.net.module if isinstance(self.net, nn.DataParallel) else self.net).state_dict()
        for k, v in merged_sd.items():
            if k in own_sd:
                own_sd[k] = v
            else:
                print(f"Warning: '{k}' not found in target model and will be skipped.")
            
        if isinstance(self.net, nn.DataParallel):
            self.net.module.load_state_dict(own_sd)
        else:
            self.net.load_state_dict(own_sd)
            


    def collect_score_loss(self, data):
        '''
        Args:
            data, dict {
                'pts': [bs, c]
    ...
    def train_score_func(self, data):
        """ One step of training """
        self.net.train()
        self.is_testing = False
        
        #data['pts_feat'] = self.net(data, mode='pts_feature')
        gf_losses = self.collect_score_loss(data)
    ...
    def train_func(self, data, pose_samples=None, gf_mode='score'):
        if gf_mode in ['score', 'energy_wo_ranking']:
            losses = self.train_score_func(data)
        elif gf_mode == 'energy':
    ...
    def inference_score_func(self, batch_size, text_prompt):
        self.is_testing = True
        self.net.eval()
        import clip
        ...
        with torch.no_grad():
            #data['pts_feat'] = self.net(data, mode='pts_feature')
            data = {}
            data["gt_pose_scale"] = torch.zeros((batch_size))
            feat_list = []
            for single_prompt in text_prompt:
                tokens = clip.tokenize(single_prompt, truncate = True).to("cuda")
                clip_feat = clip_model.encode_text(tokens).float()
                feat_list.append(clip_feat)
            data["clip_feat"] = torch.cat(feat_list, dim=0)
    ...
    def inference_hhoi_naive(self, batch_size, h1o_text_prompt, h1h2_text_prompt):
        self.is_testing = True
        self.net.eval()
        import clip
        clip_model, clip_preprocess = clip.load("ViT-B/32", device="cuda",jit=False)
        ...
        with torch.no_grad():
            ...
            hoi_feat_list = []
            for single_prompt in h1o_text_prompt:
                tokens = clip.tokenize(single_prompt, truncate = True).to("cuda")
                clip_feat = clip_model.encode_text(tokens).float()
                hoi_feat_list.append(clip_feat)
            data["hoi_clip_feat"] = torch.cat(hoi_feat_list, dim=0)
            
            hhi_feat_list = []
            for single_prompt in h1h2_text_prompt:
                tokens = clip.tokenize(single_prompt, truncate = True).to("cuda")
                clip_feat = clip_model.encode_text(tokens).float()
                hhi_feat_list.append(clip_feat)
            data["hhi_clip_feat"] = torch.cat(hhi_feat_list, dim=0)
            
            hoi_x_list = []
            hhi_x_list = []
            sampler_mode_list = self.cfg.sampler_mode
            for sampler in sampler_mode_list:
                hoi_x, hhi_x = self.net(data, mode=f'{sampler}')
                hoi_x_list.append(hoi_x)
                hhi_x_list.append(hhi_x)

        return hoi_x_list, hhi_x_list
    
    def inference_hhoi_exp(self, batch_size, h1o_text_prompt, h2o_text_prompt, h1h2_text_prompt):
        self.is_testing = True
        self.net.eval()
        import clip
        clip_model, clip_preprocess = clip.load("ViT-B/32", device="cuda",jit=False)
        ...
        with torch.no_grad():
            ...
            for single_prompt in h1o_text_prompt:
                tokens = clip.tokenize(single_prompt, truncate = True).to("cuda")
                clip_feat = clip_model.encode_text(tokens).float()
                h1oi_feat_list.append(clip_feat)
            data["h1oi_clip_feat"] = torch.cat(h1oi_feat_list, dim=0)
            
            h2oi_feat_list = []
            for single_prompt in h2o_text_prompt:
                tokens = clip.tokenize(single_prompt, truncate = True).to("cuda")
                clip_feat = clip_model.encode_text(tokens).float()
                h2oi_feat_list.append(clip_feat)
            data["h2oi_clip_feat"] = torch.cat(h2oi_feat_list, dim=0)
            ...
            sampler_mode_list = self.cfg.sampler_mode
            for sampler in sampler_mode_list:
                in_process_sample, res = self.net(data, mode=f'{sampler}')
                in_process_sample_list.append(in_process_sample)
                res_list.append(res)

        return res_list, in_process_sample_list
    
    def inference_hhoi(self, batch_size, human_list, sorted_human_list, hhi_pair_list, human_non_adjacent_info, hoi_text_prompt, hhi_text_prompt):
        self.is_testing = True
        self.net.eval()
        import clip
        clip_model, clip_preprocess = clip.load("ViT-B/32", device="cuda",jit=False)
        ...
        with torch.no_grad():
            data = {}
            data["gt_pose_scale"] = torch.zeros((batch_size))
            data["human_list"] = human_list
            data["sorted_human_list"] = sorted_human_list
            data["hhi_pair_list"] = hhi_pair_list
            data["human_non_adjacent_info"] = human_non_adjacent_info
            data['num_hoi'] = len(human_list)
            data['num_hhi'] = len(hhi_pair_list)
            
            hoi_feat_list = []
            for single_prompt in hoi_text_prompt:
                tokens = clip.tokenize(single_prompt, truncate = True).to("cuda")
                clip_feat = clip_model.encode_text(tokens).float()
                hoi_feat_list.append(clip_feat)
            data["hoi_clip_feat"] = torch.cat(hoi_feat_list, dim=0)
            ...
            hhi_feat_list = []
            for single_prompt in hhi_text_prompt:
                tokens = clip.tokenize(single_prompt, truncate = True).to("cuda")
                clip_feat = clip_model.encode_text(tokens).float()
                hhi_feat_list.append(clip_feat)
            data["hhi_clip_feat"] = torch.cat(hhi_feat_list, dim=0)
    ...
    def refinement_Rts(self, random_samples):
        self.is_testing = True
        self.net.eval()
        with torch.no_grad():
            transform_data = random_samples['transform']
            base_rotation = torch.as_tensor(transform_data[:, :, :3], dtype=torch.float32).contiguous().to(self.net.device)
            base_translation = torch.as_tensor(transform_data[:, :, 3], dtype=torch.float32).contiguous().to(self.net.device)
            base_scale = torch.as_tensor(transform_data[:, :, 4], dtype=torch.float32).contiguous().to(self.net.device)
            target_rotation = torch.as_tensor(transform_data[:, :, 5:8], dtype=torch.float32).contiguous().to(self.net.device)
            target_translation = torch.as_tensor(transform_data[:, :, 8], dtype=torch.float32).contiguous().to(self.net.device)
            target_scale = torch.as_tensor(transform_data[:, :, 9], dtype=torch.float32).contiguous().to(self.net.device)
            
            base_rot = pytorch3d.transforms.matrix_to_rotation_6d(base_rotation.permute(0, 2, 1)).reshape(base_rotation.shape[0], -1)
            target_rot = pytorch3d.transforms.matrix_to_rotation_6d(target_rotation.permute(0, 2, 1)).reshape(target_rotation.shape[0], -1)
            
            #data['pts_feat'] = self.net(data, mode='pts_feature')
            data = {}
            data["gt_pose_scale"] = torch.cat([base_rot.float(), base_translation.float(), base_scale.float(), target_rot.float(), target_translation.float(), target_scale.float()], dim=-1)
            
            sampler_mode_list = self.cfg.sampler_mode
            in_process_sample_list = []
            res_list = []
            for sampler in sampler_mode_list:
                in_process_sample, res = self.net(data, mode=f'sdedit', T0=self.cfg.T0)
                in_process_sample_list.append(in_process_sample)
                res_list.append(res)
    ...
    def update_learning_rate(self):
        ...
        elif not self.optimizer.param_groups[-1]['lr'] < 7e-4:
            self.scheduler.step()
