...
if __name__ == "__main__":
    ...
    save_dir = os.path.join(args.inference_dir, "mesh_outputs_per_obj")
    ...
    human_color_per_number = True
    
    #for i in tqdm(range(len(transform))):
    for i in tqdm(range(10)):
        scene_dir = os.path.join(save_dir, f"{i}")
        os.makedirs(scene_dir, exist_ok=True)
        
        asset = trimesh.load(f"{args.data_dir}/asset.obj")
        asset.export(f"{scene_dir}/asset.obj")
        ...
        for h_idx in range(num_human):
            ...
            human.export(f"{scene_dir}/human_{h_idx}.obj")
